...
def test_transfer_overdraw_attempt():
    ...
    transfer = 500
    assert checking.transfer(transfer) == False
...
def test_transfer_sufficient_funds():
    ...
    checking.deposit(deposit)
    transfer = 200
    assert checking.transfer(transfer)
...
def test_transfer_ledger_update_source():
    ...
    transfer = 200
    checking.transfer(transfer)
    length_after_transfer = len(checking.ledger)
    last_item = checking.ledger[-1]
    assert length_after_transfer == length_after_deposit + 1 and last_item["amount"] < 0
...
def test_transfer_ledger_update_destination():
    ...
    assert length_after_transfer == length_before_transfer + 1 and last_item["amount"] == transfer
